...
const cartSlice = createSlice({
    ...
    reducers: {
        addProduct: (state, action) => {
            let item = state.productsCart.find((item) => item.id === action.payload.id && item.color===action.payload.color && item.size ===action.payload.size);
            if (state.productsCart.includes(item)) {
                let itemm = state.productsCart.find((item) => item.id === action.payload.id);
                item.price = itemm.price + ((itemm.price * action.payload.quantity) / itemm.quantity);
...
export const { addProduct, removeProduct, removeOne, addOne } = cartSlice.actions
